...
class Application:
    ...
    def set_timer(self, seconds):
        ...
        if not self.timer_running:
            ...
            self.timer_warning_sound_played = False  # Reinicia a flag do som de aviso
            self.timer_end_sound_played = False  # Reinicia a flag do som de término
        else:
            self.end_time = time.time() + seconds
            self.timer_running = True
            self.timer_paused = False
    ...
    def update_timer(self):
        if self.timer_running:
            ...
            if remaining_time <= 0:
                ...
                self.timer_stop_button.config(state=tk.DISABLED)
                self.timer_add_10_button.config(state=tk.DISABLED)
            elif remaining_time <= 10 and not self.timer_warning_sound_played:
